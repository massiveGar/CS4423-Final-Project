...
public class TargetController : MonoBehaviour
{
    ...
    void OnEnable() {
        GameController.OnTarget += TargetEnemy;
    }
    void OnDisable() {
        GameController.OnTarget -= TargetEnemy;
    }
    ...
    // Control the list of enemies on screen
    ...
    public void RemoveInvisibleEnemy(GameObject enemy) {
        if(target == enemy) {
            ...
            GameController.Instance.SignalTargetChanged();
        }
    ...
    // Inflict power damage on target, return false if there is no target    
    public bool AttackTarget(int power) {
        if(target == null) {
            return false;
        }
        target.GetComponent<DefaultEnemyAI>().TakeDamage(power);
        return true;
    }
}
